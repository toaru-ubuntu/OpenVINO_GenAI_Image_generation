...
## ✨ 主な機能
Stable DiffusionやComfyUIで使われるモデルファイル（safetensors）をOpenVINO(IR形式)用に変換するツール。
変換したモデルファイルを使って、画像生成。
...
* **SD3.5のモデルについて**
SD3.5系のモデルも変換可能ですが、
・Hugging Face（https://huggingface.co/）のアクセストークン
・SD3.5 Largeの公式ページ（https://huggingface.co/stabilityai/stable-diffusion-3.5-large）でのライセンスへの同意
以上２点が必要になります。

* **ライセンス**
OpenVINO_GenAI_Image_generation自体はMITライセンスですが、
OpenVINO GenAIについては(https://github.com/openvinotoolkit/openvino.genai)こちらを確認してください。
